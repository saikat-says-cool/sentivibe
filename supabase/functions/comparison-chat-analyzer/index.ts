  ...
  try {
    ...
    if (effectiveDeepSearchMode) {
      ...
      if (fetchExternalContextResponse.error) {
        console.warn("Error fetching external context for comparison chat:", fetchExternalContextResponse.error);
    ...
    You are SentiVibe AI, an insightful, factual, and transparent conversational assistant specializing in comparative video analysis. Your core mission is to decode the voice of the crowd, transforming unstructured online reactions into clear, actionable insight by comparing two YouTube videos. Maintain a professional clarity with warm confidence and data-science credibility.
    ...
    **Response Guidelines:**
    ...
    switch (selectedPersona) {
      case 'therapist':
        ...
        break;
      case 'storyteller':
        personaSpecificInstructions = `You are SentiVibe AI, a captivating storyteller. Your responses should be imaginative, descriptive, and engaging, weaving information into narratives or using vivid language. You can tell stories related to the videos' themes or create new ones, always ensuring they are relevant to the user's query and the comparison context. Use evocative language and narrative structures.`;
        break;
      case 'motivation':
      ...
      default:
        personaSpecificInstructions = `You are SentiVibe AI, your friendly and approachable chat companion. Your goal is to provide helpful, easy-to-understand insights and engage in a supportive conversation about the video comparison. Explain complex topics simply and offer encouragement, using a warm and helpful tone.`;
        break;
    }

    // Add custom comparative QA results to the context if available
    let customComparativeQaContext = "";
    if (comparisonResult.customComparativeQaResults && comparisonResult.customComparativeQaResults.length > 0) {
      customComparativeQaContext = "\n\n--- Pre-generated Comparative Q&A Results ---\n";
      comparisonResult.customComparativeQaResults.forEach((qa: any, index: number) => {
    ...
    const fullContext = `
    --- Comparison Analysis Context ---
    Comparison Title: "${comparisonResult.comparisonTitle}"
    Comparison Meta Description: "${comparisonResult.comparisonMetaDescription}"
    Comparison Keywords: ${comparisonResult.comparisonKeywords.length > 0 ? comparisonResult.comparisonKeywords.join(', ') : 'None'}
    
    Video A Title: "${comparisonResult.videoATitle}"
    Video A Link: "${comparisonResult.videoALink}"
    Video A Top Comments:
    ${comparisonResult.videoARawCommentsForChat ? comparisonResult.videoARawCommentsForChat.slice(0, 10).map((comment: string, index: number) => `${index + 1}. ${comment}`).join('\n') : 'No comments available.'}

    Video B Title: "${comparisonResult.videoBTitle}"
    Video B Link: "${comparisonResult.videoBLink}"
    Video B Top Comments:
    ${comparisonResult.videoBRawCommentsForChat ? comparisonResult.videoBRawCommentsForChat.slice(0, 10).map((comment: string, index: number) => `${index + 1}. ${comment}`).join('\n') : 'No comments available.'}
    ...
    for (const currentLongcatApiKey of longcatApiKeys) {
      longcatResponse = await fetch(longcatApiUrl, {
        ...
        body: JSON.stringify({
          ...
          stream: false, // <--- Reverted to false for non-streaming
        }),
      });
      ...
      if (longcatResponse.ok) {
        ...
        console.warn(`Longcat AI API key hit quota limit for comparison chat. Trying next key.`);
        continue;
      }
      break;
    }
    ...
    if (!longcatResponse || !longcatResponse.ok) {
      ...
      console.error('Longcat AI API error for comparison chat:', errorData);
    ...
    if (!longcatData.choices || longcatData.choices.length === 0 || !longcatData.choices[0].message || !longcatData.choices[0].message.content) {
      console.error('Longcat AI API returned unexpected structure:', longcatData);
      throw new Error('Longcat AI API returned an empty or malformed response.');
    }
    const aiResponseContent = longcatData.choices[0].message.content;

    return new Response(JSON.stringify({ aiResponse: aiResponseContent }), {
      headers: { ...corsHeaders, 'Content-Type': 'application/json' },
      status: 200,
    });
